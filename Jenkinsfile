    ...
    stages {
        ...
        stage('Git Checkout') {
            steps {
                git branch: 'main',
                    ...
                    url: 'https://github.com/sandeshjamdar20/aws-2-tier-project.git'
            }
        }
        ...
        stage('Clone Helm Chart for CD') {
            steps {
                git branch: 'main',
                    credentialsId: 'github-cred',
                    url: 'https://github.com/sandeshjamdar20/aws-2-tier-helm-chart.git'
            }
        }
            ...
            steps {
                withCredentials([usernamePassword(
                    ...
                    sh """
                        ...
                        git config user.email "sandeshjamdar51@gmail.com"
                        git config user.name "${GIT_USER_NAME}"

                        echo "=== BEFORE ==="
                        cat helm-chart/values.yaml

                        sed -i "s|image: 609898225296.dkr.ecr.us-east-1.amazonaws.com/frontend-repo:.*|image: 657001761946.dkr.ecr.us-east-1.amazonaws.com/frontend-repo:${IMAGE_TAG}|" helm-chart/values.yaml
                        sed -i "s|image: 609898225296.dkr.ecr.us-east-1.amazonaws.com/backend-repo:.*|image: 657001761946.dkr.ecr.us-east-1.amazonaws.com/backend-repo:${IMAGE_TAG}|" helm-chart/values.yaml
